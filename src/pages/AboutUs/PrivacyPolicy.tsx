...
export default function PrivacyPolicy() {
  ...
  const { contentData, isLoading, error, refetch } = useContentData('privacy-policy');
  ...
  const handleSave = async () => {
    try {
      if (privacyText.trim() && privacyText !== '<p></p>' && privacyText !== '<p><br></p>') {
        if (contentData?._id) {
  ...
  useEffect(() => {
    ...
    console.log('PrivacyPolicy - useEffect triggered:', { contentData, isLoading, editorLoaded });
    
    if (contentData?.content) {
      // Content is already decoded in the Redux store
      console.log('PrivacyPolicy - Setting content:', contentData.content);
      setPrivacyText(contentData.content);
      setIsCreating(false);
      
      // Update editor content if editor is ready
      if (editorRef.current && editorLoaded) {
        console.log('PrivacyPolicy - Setting editor data immediately');
        editorRef.current.setData(contentData.content);
      }
    } else if (!isLoading && !contentData) {
      // No content exists, prepare for creation
      console.log('PrivacyPolicy - No content found, setting creation mode');
      setPrivacyText('');
      setIsCreating(true);
  ...
  useEffect(() => {
    if (editorRef.current && editorLoaded && privacyText) {
      console.log('PrivacyPolicy - Editor ready, setting content:', privacyText);
      setTimeout(() => {
        editorRef.current.setData(privacyText);
      }, 200);
    }
  }, [editorLoaded, privacyText]);
  ...
  if (isLoading) {
    return (
      <div>
        <PageMeta
          title="Privacy Policy | TailAdmin - Video Player Admin Dashboard"
          description="Privacy Policy page for TailAdmin - Video Player Admin Dashboard"
        />
        ...
        <div className="w-full max-w-4xl">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8">
            ...
            {isCreating && (
              <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                <p className="text-blue-800 dark:text-blue-200 text-sm">
                  No existing privacy policy found. Create new privacy policy content below.
                </p>
              </div>
            )}
                ...
                {editorLoaded ? (
                  <div className="ck-editor-container">
                    ...
                    <CKEditor
                      ...
                      data=""
                      onReady={(editor: any) => {
                        console.log('PrivacyPolicy - Editor is ready to use!', editor);
                        editorRef.current = editor;
                        
                        // Set initial content if available
                        if (privacyText) {
                          console.log('PrivacyPolicy - Setting initial content in onReady:', privacyText);
                          setTimeout(() => {
                            editor.setData(privacyText);
                          }, 100);
                        }
            ...
            <div className="flex justify-between items-center pt-4">
              <button
                onClick={handleSave}
                disabled={!privacyText.trim() || privacyText === '<p></p>' || privacyText === '<p><br></p>' || isSaving}
