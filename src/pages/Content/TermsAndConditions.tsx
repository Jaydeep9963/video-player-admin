...
export default function TermsAndConditions() {
  ...
  const { contentData, isLoading, error, refetch } = useContentData('terms-and-conditions');
  ...
  const handleSave = async () => {
    try {
      if (termsText.trim() && termsText !== '<p></p>' && termsText !== '<p><br></p>') {
        if (contentData?._id) {
      ...
      console.log('Error saving terms and conditions:', error);
  ...
  useEffect(() => {
    console.log('TermsAndConditions - useEffect triggered:', { contentData, isLoading, editorLoaded });
    
    if (contentData?.content) {
      console.log('TermsAndConditions - Setting content:', contentData.content);
      setTermsText(contentData.content);
      setIsCreating(false);
      
      // Update editor content if editor is ready
      if (editorRef.current && editorLoaded) {
        console.log('TermsAndConditions - Setting editor data immediately');
        editorRef.current.setData(contentData.content);
      }
    } else if (!isLoading && !contentData) {
      console.log('TermsAndConditions - No content found, setting creation mode');
      setTermsText('');
      setIsCreating(true);
  ...
  useEffect(() => {
    if (editorRef.current && editorLoaded && termsText) {
      console.log('TermsAndConditions - Editor ready, setting content:', termsText);
      setTimeout(() => {
        editorRef.current.setData(termsText);
      }, 200);
    }
  }, [editorLoaded, termsText]);
  ...
  if (isLoading) {
    return (
      <div>
        <PageMeta
          title="Terms and Conditions | TailAdmin - Video Player Admin Dashboard"
          description="Terms and Conditions page for TailAdmin - Video Player Admin Dashboard"
        />
        ...
        <div className="w-full max-w-4xl">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8">
            ...
            {isCreating && (
              <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                <p className="text-blue-800 dark:text-blue-200 text-sm">
                  No existing terms and conditions found. Create new terms and conditions content below.
                </p>
              </div>
            )}
                ...
                {editorLoaded ? (
                  <div className="ck-editor-container">
                    ...
                    <CKEditor
                      ...
                      data=""
                      onReady={(editor: any) => {
                        console.log('TermsAndConditions - Editor is ready to use!', editor);
                        editorRef.current = editor;
                        
                        // Set initial content if available
                        if (termsText) {
                          console.log('TermsAndConditions - Setting initial content in onReady:', termsText);
                          setTimeout(() => {
                            editor.setData(termsText);
                          }, 100);
                        }
            ...
            <div className="flex justify-between items-center pt-4">
              <button
                onClick={handleSave}
                disabled={!termsText.trim() || termsText === '<p></p>' || termsText === '<p><br></p>' || isSaving}
